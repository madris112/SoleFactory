...
function Home(props) {
  ...
  let ThirdQuery = null;
  if (searchParams.get("category")) SecondQuery = searchParams.get("category");
  ...
  useEffect(async () => {
    if (category !== null || ThirdQuery) {
      ...
      if (category !== null) query = "?searchname=" + category + "&category=1";
      if (ThirdQuery !== null)
  ...
  useEffect(async () => {
    ...
    await axios
      .get("http://localhost:4000/product/search", {
        params: { searchname: FirstQuery, category: SecondQuery },
        header,
      })
      ...
      axios
        ...
        .then((response) => {
          if (response.data.retuser) {
            ...
            if (
              response.data.retuser &&
              response.data.retuser.IsActivated === "0"
            ) {
  ...
  function handleClick(e) {
    ...
    let query = null;
    if (searchname != null) query = "?searchname=" + searchname;
